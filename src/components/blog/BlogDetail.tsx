...
export default function BlogDetail({ blog }: { blog: Blog }) {
  ...
  return (
    <main className="bg-[var(--surface-0)] text-white overflow-hidden">
      <section className="relative overflow-hidden bg-[var(--surface-1)] px-5 pb-16 pt-[120px] text-white sm:px-8 sm:pb-20 sm:pt-[140px] lg:px-10 lg:pb-24 lg:pt-[160px] xl:px-16 min-h-[100svh] flex flex-col justify-center">
        <motion.div 
          initial="hidden"
          animate="visible"
          variants={fadeIn}
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[var(--surface-0)] via-[var(--surface-0)]/80 to-[var(--surface-0)]/20" />
        ...
        >
          <motion.div variants={fadeInUp}>
            <Link
              href="/blog"
              className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em] text-[var(--primary)] backdrop-blur-md transition duration-300 hover:border-[var(--primary)]/40 hover:bg-white/10"
            >
              <ArrowLeft size={16} />
              Back to Blog
            </Link>
          ...
          <motion.div variants={fadeInUp} className="flex flex-wrap items-center gap-3 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.16em] text-white/50 sm:gap-4">
            <span className="rounded-full border border-[var(--primary)]/30 bg-[var(--primary)]/10 px-3 py-1.5 text-[var(--primary)] backdrop-blur-sm">{blog.category}</span>
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)] lg:items-end">
            ...
            <motion.div variants={fadeInUp} className="rounded-[2rem] border border-white/10 bg-[var(--surface-1)]/60 p-6 backdrop-blur-xl sm:p-8 shadow-2xl">
      ...
      <section className="-mt-12 px-5 pb-16 sm:px-8 sm:pb-20 lg:px-10 lg:pb-24 xl:px-16 relative z-20">
        <div className="mx-auto w-full max-w-[1280px]">
          <motion.div 
            ...
            viewport={{ once: true }}
            variants={scaleIn}
            className="overflow-hidden rounded-[2.5rem] border border-white/10 bg-[var(--surface-1)] shadow-2xl"
          >
            <div className="relative aspect-[16/8] w-full bg-[var(--surface-0)]">
              <Image
                src={blog.image}
                alt={blog.title}
                fill
      ...
      <section className="px-5 pb-20 sm:px-8 sm:pb-32 lg:px-10 lg:pb-40 xl:px-16 overflow-hidden relative">
        ...
        >
          <motion.article variants={fadeInUp} className="rounded-[2.5rem] border border-white/10 bg-[var(--surface-1)]/80 backdrop-blur-md px-6 py-10 shadow-2xl sm:px-10 sm:py-12 lg:px-12 lg:py-14">
            <BlogMarkdown content={blog.content} />
          </motion.article>

          <motion.aside variants={fadeInUp} className="space-y-8">
            <div className="rounded-[2rem] border border-white/10 bg-[var(--surface-1)] p-8 shadow-2xl">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {blog.tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[13px] font-medium text-white/70 transition-colors hover:border-[var(--primary)]/40 hover:text-white"
                  >
                    <Tag size={13} />
                    {tag}
                  </span>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-gradient-to-b from-[var(--surface-1)] to-[#0A1D3A] p-8 text-white shadow-[0_0_40px_rgba(56,189,248,0.1)] relative overflow-hidden">
               <div className="absolute top-0 right-0 h-32 w-32 bg-[radial-gradient(circle,rgba(56,189,248,0.2)_0%,transparent_70%)] blur-xl" />
              <p className="text-[12px] font-bold uppercase tracking-[0.2em] text-[var(--accent)] relative z-10">
                Let's Build
              </p>
              <h2 className="mt-4 text-[1.8rem] font-bold leading-[1.1] tracking-[-0.03em] relative z-10">
                Ready to turn your idea into a <span className="text-[var(--primary)]">serious product?</span>
              </h2>
              ...
              <div className="mt-8 flex flex-col gap-4 relative z-10">
                <Link
                  href="/contact"
                  className="rounded-full bg-gradient-to-r from-[var(--primary)] to-[var(--accent)] px-6 py-4 text-center text-[15px] font-bold text-[var(--surface-0)] transition-transform hover:scale-[1.02]"
                >
                  Free Consultation
                </Link>
                <Link
            ...
            >
              {relatedBlogs.map((item) => (
                <motion.div key={item.slug} variants={card3D} whileHover="hover">
                  <Link
                    href={`/blog/${item.slug}`}
                    className="group flex flex-col h-full overflow-hidden rounded-[2rem] border border-white/10 bg-[var(--surface-1)] shadow-2xl transition-all duration-300 hover:border-[var(--primary)]/40 hover:shadow-[0_0_30px_rgba(56,189,248,0.15)]"
                  >
                    ...
                    <div className="p-8 flex-1 flex flex-col">
                      ...
                      <span className="mt-auto pt-6 inline-flex items-center gap-2 text-[14px] font-bold text-[var(--accent)] transition-all duration-300 group-hover:gap-3">
                        Read Article
                        <ArrowRight size={16} />
                      </span>
                    </div>
